...
//					System.out.println("encontr� con qu� matchear");
	...
	public List<String> getTopLevelPackagesForVersion(String version){
		...
		//if we are here, we do not have neither the version nor the .jar, so we can only na�vely check for the first level packages ... re using Version code
		
		logger.info("Getting na�ve top level packages for: "+version);
		...
		for(Entry<String,Map<String,double[]>> e : smellEvolution.entrySet()){
			...
			if(Double.isFinite(scores.get(scores.keySet().iterator().next())[versionN])) //if the score is finite
				smellsInVersion.addAll(Arrays.asList(e.getKey().substring(e.getKey().indexOf("_")).split(";"))); //TODO: ESTO ESTÁ MAL!!!
		}
